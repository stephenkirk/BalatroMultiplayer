...
MP.ReworkCenter({
	...
	use = function(self, card, area, copier)
		...
		G.E_MANAGER:add_event(Event({
			...
			func = function()
				play_sound("timpani")
				if MP.should_use_the_order() then -- this only matters if order exists
					local done = false
					while not done do -- AHHH we have to do so much boilerplate
						...
						if
							...
							and G.P_CENTERS[center].perishable_compat
						then
							card.stickers.perishable = true
							done = false
						end

						if G.GAME.modifiers.enable_rentals_in_shop and rental_poll > 0.7 then
