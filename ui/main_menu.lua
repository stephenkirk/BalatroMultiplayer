				...
				nodes = {
					{
						...
						config = {
							...
							minw = 11,
							maxw = 11,
							minh = math.max(2, ruleset_desc_lines) * 0.35,
						},
						nodes = {
						...
						nodes = {
							{
								...
								nodes = {
									{
										n = G.UIT.T,
										config = { text = localize("b_next"), scale = 0.5, colour = G.C.UI.TEXT_LIGHT },
									},
								},
							},
						},
...
function G.FUNCS.ruleset_switch_tabs(args)
	...
	tabs_wrap.config.object = UIBox({
		definition = tabs_object.config.is_bans_tabs
				and G.UIDEF.ruleset_tabs_definition(callback_args.ruleset, true, active_tab_idx)
			or G.UIDEF.ruleset_tabs_definition(callback_args.ruleset, false, active_tab_idx),
	...
	for k, v in ipairs({
		{
			...
			obj_ids = is_banned_tab and copy_all_banned_ids(MP.DECK.BANNED_JOKERS, ruleset.banned_jokers)
				or ruleset.reworked_jokers,
		},
		{
			type = localize("b_stat_consumables"),
			obj_ids = is_banned_tab and copy_all_banned_ids(MP.DECK.BANNED_CONSUMABLES, ruleset.banned_consumables)
				or ruleset.reworked_consumables,
		},
		{
			type = localize("b_vouchers"),
			obj_ids = is_banned_tab and copy_all_banned_ids(MP.DECK.BANNED_VOUCHERS, ruleset.banned_vouchers)
				or ruleset.reworked_vouchers,
		},
		{
			type = localize("b_enhanced_cards"),
			obj_ids = is_banned_tab and copy_all_banned_ids(MP.DECK.BANNED_ENHANCEMENTS, ruleset.banned_enhancements)
				or ruleset.reworked_enhancements,
		},
		{
			type = localize("k_other"),
			obj_ids = is_banned_tab and {
				tags = copy_all_banned_ids(MP.DECK.BANNED_TAGS, ruleset.banned_tags),
				blinds = copy_all_banned_ids(MP.DECK.BANNED_BLINDS, ruleset.banned_blinds),
			} or { tags = ruleset.reworked_tags or {}, blinds = ruleset.reworked_blinds or {} },
		},
	}) do
		v.idx = k
	...
	local function get_ruleset_cardarea(obj_ids, width, height)
		...
		if #obj_ids > 0 then
			...
			for k, v in ipairs(obj_ids) do
				...
				if #card_rows[_row] > max_width then
					max_width = #card_rows[_row]
				end
			end
				...
				if not G.CONTROLLER.dragging.target or G.CONTROLLER.using_touch then
					if not temp_blind.hovering and temp_blind.states.visible then
						...
						temp_blind.config.h_popup_config =
							{ align = "cl", offset = { x = -0.1, y = 0 }, parent = temp_blind }
