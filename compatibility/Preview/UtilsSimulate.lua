...
function FN.SIM.adjust_field_with_range(adj_func, field, mod_func, exact_value, min_value, max_value)
	if not exact_value then
		error("Cannot adjust field, exact_value is missing.")
	end
...
function FN.SIM.is_suit(card_data, suit, ignore_scorability)
	if card_data.debuff and not ignore_scorability then
		return
	end
	...
	if next(find_joker("Smeared Joker")) then
		...
		if is_card_suit_light == is_check_suit_light then
			return true
		end
	end
